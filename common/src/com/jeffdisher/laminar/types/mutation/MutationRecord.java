 ...
 * <li>Consequences:  These are generated by every node in the cluster when processing a mutation (when a node decides or is told
 ...
 *  these, the mapping from the mutation to consequence is deterministic so every node derives the same answer.</li>
 * </ul>
 * It is worth noting that that processing a single mutation normally produces a single consequence but programmable topics
 * can work differently where a mutation can result in 0 or several consequences being generated.
 * 
