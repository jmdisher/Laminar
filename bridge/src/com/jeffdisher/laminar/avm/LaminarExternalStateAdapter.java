		...
		if (executionResult.getResult().transactionStatus.isSuccess()) {
			...
			for (OutputTuple tuple : _outputList) {
				Consequence record = (null != tuple.value)
						? Consequence.put(termNumber, _globalOffset, nextLocalOffset, clientId, clientNonce, tuple.key, tuple.value)
						: Consequence.delete(termNumber, _globalOffset, nextLocalOffset, clientId, clientNonce, tuple.key);
				nextLocalOffset += 1;
				result.add(record);
			}
		} else {
