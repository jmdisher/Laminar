 ...
 * This is created for every invocation so it can be specific to the context of each intention.
...
public class LaminarExternalStateAdapter implements IExternalState {
	...
	private final long _intentionOffset;
	private final byte[] _valueForPut;

	/**
	 * Creates an instance to handle a single intention execution.
	 * 
	 * @param context The code and data of the contract.
	 * @param intentionOffset The global offset of the intention being executed.
	 * @param valueForPut The value associated with a PUT operation (since only the key can be passed in).
	 */
	public LaminarExternalStateAdapter(TopicContext context, long intentionOffset, byte[] valueForPut) {
		_context = context;
		_outputList = new LinkedList<>();
		_intentionOffset = intentionOffset;
		_valueForPut = valueForPut;
	}

	/**
	 * Called after an invocation has completed to map any results into things Laminar can interpret.
	 ...
	 * @param termNumber The term number where the intention committed.
	 * @param initialLocalOffset The first local offset to apply to any generated Consequences.
	 * @param clientId The client who send the intention.
	 * @param clientNonce The client's nonce associated with the intention.
	 * @return The list of Consequences generated by the transaction or null, if there was failure.
	 */
	public List<Consequence> createOutputConsequences(FutureResult executionResult, long termNumber, long initialLocalOffset, UUID clientId, long clientNonce) {
		List<Consequence> result = null;
		if (executionResult.getResult().transactionStatus.isSuccess()) {
			...
			for (OutputTuple tuple : _outputList) {
				Consequence record = (null != tuple.value)
						? Consequence.put(termNumber, _intentionOffset, nextLocalOffset, clientId, clientNonce, tuple.key, tuple.value)
						: Consequence.delete(termNumber, _intentionOffset, nextLocalOffset, clientId, clientNonce, tuple.key);
				nextLocalOffset += 1;
				result.add(record);
			}
		} else {
	...
	public long getBlockNumber() {
		// We will treat the intention global offset as the "block number".
		return _intentionOffset;
	}
