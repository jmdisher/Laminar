...
public class Helpers {
	 ...
	 */
	public static MutationRecord convertClientMessageToMutation(ClientMessage message, UUID clientId, long mutationOffsetToAssign) {
		...
		case TEMP: {
			...
			converted = MutationRecord.generateRecord(MutationRecordType.TEMP, mutationOffsetToAssign, clientId, message.nonce, contents);
		}
			break;
		case POISON: {
			// This is just for initial testing:  send the received, log it, and send the commit.
		...
		case UPDATE_CONFIG: {
			...
			converted = MutationRecord.generateRecord(MutationRecordType.UPDATE_CONFIG, mutationOffsetToAssign, clientId, message.nonce, newConfig.serialize());
		}
			break;
		default:
